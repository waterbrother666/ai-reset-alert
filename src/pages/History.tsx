...
function HistoryRow({ record, expanded, focused, onToggle }: { record: SignalRecord; expanded: boolean; focused: boolean; onToggle: () => void }) {
  return <div id={`history-${record.id}`} className={`border-b border-[var(--color-border)] px-4 py-4 last:border-0 hover:bg-[var(--color-surface-2)]/50 sm:px-5 ${focused ? 'bg-[var(--color-brand-soft)]' : ''}`}>
    <TiboPost record={record} compact expanded={expanded} onToggle={onToggle} />
  </div>
}

export function History({ records, loading, error, focusRecordId, onReload, onBack }: { records: SignalRecord[]; loading: boolean; error: string | null; focusRecordId?: string | null; onReload: () => void; onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const index = filtered.findIndex((record) => record.id === focusRecordId)
    if (index >= visible) setVisible(index + 1)
    window.requestAnimationFrame(() => document.getElementById(`history-${focusRecordId}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' }))
  }, [filtered, focusRecordId, visible])
  return <div className="mx-auto flex max-w-[1080px] flex-col gap-4">
    ...
    <Card className="overflow-hidden">{loading ? <div>{Array.from({ length: 5 }).map((_, index) => <div key={index} className="flex gap-3 px-5 py-4"><Skeleton className="h-10 w-10 shrink-0 rounded-full" /><div className="flex-1 space-y-2"><Skeleton className="h-4 w-40" /><Skeleton className="h-4 w-3/4" /><Skeleton className="h-4 w-1/2" /></div></div>)}</div>
      ...
      : <>{shown.map((record) => <HistoryRow key={record.id} record={record} expanded={expandedId === record.id} focused={focusRecordId === record.id} onToggle={() => setExpandedId((id) => id === record.id ? null : record.id)} />)}{filtered.length > visible && <div className="p-3"><Button variant="ghost" className="mx-auto w-full" onClick={() => setVisible((value) => value + 50)}>加载更多</Button></div>}</>}</Card>
  </div>
}
